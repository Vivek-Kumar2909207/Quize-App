...
try:
    class QuizeApp():  
        def test_details(self):
            ...
            minutes_box=tk.Entry(self.test_root,text="HH",textvariable=self.min,font=("Arial",15))
        ...
        def check_details(self):
            ...
            try:
                if self.stdname.get().startswith(l) or self.stdname.get().endswith(l):
                    ...
                    self.hours = int(self.hrs.get().strip())
                    self.minutes = int(self.min.get().strip())
        ...
        def countdown(self,hours,minutes):
            time_label=tk.Label(self.root,font=("Times New Roman",30),width=5,background="light blue",borderwidth=2,relief="solid",fg="red")
            time_label.place(x=self.root.winfo_screenwidth()*.05,y=20)
